...
public class ModItemTier implements IItemTier {
    ...
    private ModItemTier(int level, int uses, float speed, float damage, int enchantmentValue,
                        Supplier<Ingredient> repairIngredient, int color) {
        this.level = level;
        this.uses = uses;
        this.speed = speed;
        this.damage = damage;
    ...
    public static void registerItemTier() {
        ...
        ourTiers.put("gold", out(3, 75, 5, 5, 4, Items.GOLD_INGOT, MaterialColor.GOLD.col));
    ...
    private static ModItemTier out(int level, int uses, float speed, float damage, int enchantmentValue,
                                   Item repairIngredient, int color) {
        return new ModItemTier(level, uses, speed, damage, enchantmentValue, () -> Ingredient.of(repairIngredient),
                color);
    }

    private static ModItemTier out(int level, int uses, float speed, float damage, int enchantmentValue,
                                   ITag<Item> repairIngredient, int color) {
        return new ModItemTier(level, uses, speed, damage, enchantmentValue, () -> Ingredient.of(repairIngredient),
                color);
    }
